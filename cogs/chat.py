        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                for line in decoded.split('\n'):
                    if line.startswith('data:'):
                        ...
                        try:
                            ...
                            logger_error.error(
                                f"Exception of type {type(json_error).__name__} occurred: {json_error}"
                            )
                            await interaction.followup.send(
                                'Looks like an error occurred. Report this to the dev please: (Beetle)\nPlease join the following Discord Server and submit the error message as a bug report:\nhttps://discord.gg/CUc9PAgUYB\n\n```Error: '
                                + str(json_error)
                                + "```",
                                ephemeral=True,
                            )
                ...
                if not accumulated_text.strip():
                    logger_error.error(
                        f"Empty response received for user {interaction.user.id}. Refreshing cookie and retrying automatically..."
                    )
                    await db.delete_cookies(user_id)
                    cookie_dict = await fetch_and_save_cookies_second_round(context, user_id)
                    ...
                    if retry_response.status_code == 200:
                        ...
                        if retry_text.strip():
                            if len(retry_text) > 2000:
                                ...
                                await thinking_msg.edit(content=retry_text)
                            return

                    await thinking_msg.edit(content="I'm having trouble responding right now. Please try again in a moment.")
                    return
                ...
                if len(accumulated_text) > 2000:
                    ...
                    await interaction.followup.send(accumulated_text)
